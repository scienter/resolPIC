...
void interpolation3D_Split_1st(Domain *D,External *Ext);
void interpolation2D_Split_2nd(Domain *D,External *Ext);
void interpolation3D_Split_2nd(Domain *D,External *Ext);
...
void interpolation3D_Yee_Pukhov_2nd(Domain *D,External *Ext);
...
void interpolation(Domain *D,External *Ext)
{
  ...
  case ((Split-1)*6+(FIRST-1)*3+2) :
    ...
    break;
  case ((Split-1)*6+(FIRST-1)*3+3) :
    interpolation3D_Split_1st(D,Ext);	//3D
    break;
	 // 2nd
  case ((Split-1)*6+(SECOND-1)*3+2) :  //2D
    interpolation2D_Split_2nd(D,Ext);
    break;
  case ((Split-1)*6+(SECOND-1)*3+3) :  //3D
    interpolation3D_Split_2nd(D,Ext);
    break;
  //Yee, Pukhov
    // 1st
  ...
  case ((Pukhov-1)*6+(FIRST-1)*3+3) :
    interpolation3D_Yee_Pukhov_1st(D,Ext);	//3D
    break;
	 // 2nd
  case ((Yee-1)*6+(SECOND-1)*3+2) :
  case ((Pukhov-1)*6+(SECOND-1)*3+2) :
    interpolation2D_Yee_Pukhov_2nd(D,Ext);	//2D
    break;
  case ((Yee-1)*6+(SECOND-1)*3+3) :
  case ((Pukhov-1)*6+(SECOND-1)*3+3) :
    interpolation3D_Yee_Pukhov_2nd(D,Ext);	//3D
    break;
  default :
               ...
               for(kk=0; kk<3; kk++)
                 for(jj=0; jj<3; jj++)
                   for(ii=0; ii<3; ii++)
                   {
                     ...
                     E2+=D->Ey[i1-1+ii][j-1+jj][k1-1+kk]*Wx[ii]*WyC[jj]*Wz[kk];
                     E3+=D->Ez[i1-1+ii][j1-1+jj][k-1+kk]*Wx[ii]*Wy[jj]*WzC[kk];
                     B1+=D->Bx[i1-1+ii][j-1+jj][k-1+kk]*Wx[ii]*WyC[jj]*WzC[kk];
                     B2+=D->By[i-1+ii][j1-1+jj][k-1+kk]*WxC[ii]*Wy[jj]*WzC[kk];
                     B3+=D->Bz[i-1+ii][j-1+jj][k1-1+kk]*WxC[ii]*WyC[jj]*Wz[kk];
                   }
