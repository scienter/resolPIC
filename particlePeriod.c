...
void MPI_TransferP_Period_Yplus(Domain *D,int istart,int iend,int jstart,int jend,int kstart,int kend,int iteration);
void MPI_TransferP_Period_Yminus(Domain *D,int istart,int iend,int jstart,int jend,int kstart,int kend,int iteration);
...
void particlePeriod(Domain D,int iteration)
{
  ...
  case 2 :
    ...
    kstart=0;           kend=1;
    if(D.periodY==ON)  {
      MPI_TransferP_Period_Yplus(&D,istart,iend,jstart,jend,kstart,kend,iteration);
      MPI_TransferP_Period_Yminus(&D,istart,iend,jstart,jend,kstart,kend,iteration);
    }  else ;
    break;
  case 3 :
    istart=D.istart;    iend=D.iend;
...
//    }  else ;
    break;
  default :
    printf("In particlePeriod, what dimenstion(%d)?\n",D.dimension);
  }
}
