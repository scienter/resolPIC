...
void storeParticle(Domain *D,int i,int j,int k,int s,double x,double y,double z,double weight,double px,double py,double pz);
     ...
     for(j=jstart; j<jend; j++)
       for(k=kstart; k<kend; k++) 
       {
         ...
         if(W>0)
         {
           ...
           while(q)  {
             storeParticle(D,i,j,k,s,q->x,0.0,0.0,q->weight,q->px,q->py,q->pz);
             qHead->next=q->next;
             q->next=NULL;
             free(q);
             q=qHead->next;
