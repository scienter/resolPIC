...
double funcNewton(Domain *D,int i,int j,int k,int s,double beta,double energy);
double funcPNewton(Domain *D,int i,int j,int k,int s,double beta);
...
void calMomentum1D(Domain *D,int s)
{
  ...
  double beta,bfbeta,fb,fPb,err,weight,energy;
  ...
  for(i=istart; i<iend; i++)
    for(j=jstart; j<jend; j++)
      for(k=kstart; k<kend; k++)
      {
        ...
        for(l=0; l<2; l++)  { Cx[l]=0.0; Cy[l]=0.0; Cz[l]=0.0; }
...
/*
          bfbeta=beta=1.0;
          err=1e5;
          fb=funcNewton(D,i,j,k,s,beta,energy);
          if(fb!=0)  {
            while(err>0.01)    {
              fb=funcNewton(D,i,j,k,s,beta,energy);
              fPb=funcPNewton(D,i,j,k,s,beta);
              if(fPb==0)  err=0.001;
              else  { beta-=fb/fPb; err=sqrt(fb*fb); }
            }
          } else;
*/
          beta=1.0;
...
double funcNewton(Domain *D,int i,int j,int k,int s,double beta,double energy)
{
  double px,py,pz,weight,sum,result;
  ptclList *p;
  ...
  while(p)  {
    weight=p->weight;
    px=p->px+beta*p->pxSam;
    py=p->py+beta*p->pySam;
    pz=p->pz+beta*p->pzSam;
    sum+=sqrt(1.0+px*px+py*py+pz*pz)*weight;
    p=p->next;
  }
  result=sum-energy;
  return result;
...
double funcPNewton(Domain *D,int i,int j,int k,int s,double beta)
{
  double px,py,pz,weight,sum,tmp;
  ptclList *p;
  ...
  while(p)  {
    ...
    py=p->py+beta*p->pySam;
    pz=p->pz+beta*p->pzSam;
    tmp=sqrt(1.0+px*px+py*py+pz*pz)*weight;
    sum+=(px*p->pxSam+py*p->pySam+pz*p->pzSam)/tmp;
    p=p->next;
  }
  return sum;
}
...
double gaussian(double sigma)
{
   double r,prob,v,random,result;
   int intRand,randRange=1000;

   if(sigma==0) result=0.0;
   else {
     r=1.0;
     prob=0.0;
     while (r>prob)  {
