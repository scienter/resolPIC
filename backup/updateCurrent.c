...
//  void MPI_TransferJ_Zminus();
  ...
  case ((FIRST-1)*3+3) :
    ...
    if(D.M>1)  {
      MPI_TransferJ_Yplus(&D,D.Jx,D.Jy,D.Jz,D.nxSub,D.nzSub+5,3);
      MPI_TransferJ_Yminus(&D,D.Jx,D.Jy,D.Jz,D.nxSub,D.nzSub+5,3);
    }  else	;
    if(D.N>1)  {
      MPI_TransferJ_Zplus(&D,D.Jx,D.Jy,D.Jz,D.nxSub,D.nySub,3);
      MPI_TransferJ_Zminus(&D,D.Jx,D.Jy,D.Jz,D.nxSub,D.nySub,3);
    }  else	;
    break;
  case ((2-1)*3+3) :
